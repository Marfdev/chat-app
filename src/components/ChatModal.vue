...
<template>
    <div class="max-w-xl mx-auto bg-white rounded-sm p-1">
        <header class="font-bold text-lg inline-flex items-center w-full px-4 ">
            ...
            <button class="bg-transparent mb-1 border-transparent text-sky-500 font-medium text-3xl" @click="signOut">
                &times; 
            </button>
        </header>
        <hr class="bg-black h-1 border-transparent">
        <div class="p-2 h-96 max-h-96 overflow-y-auto">
        ...
        <footer>
            <form class="inline-flex w-full py-4" @submit.prevent="messageHandler">
                ...
                <button type="submit" class="rounded-full bg-sky-500 p-2 mr-2"> <SendIcon /> </button>
            </form>
        </footer>
    </div>
</template>